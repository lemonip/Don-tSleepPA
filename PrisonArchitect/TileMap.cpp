...
HRESULT TileMap::init()
{
	...
	_terrainImg = ImageManager::GetInstance()->FindImage("terrain");
...
void TileMap::update()
{
	...
	{
		cout << _vTile[_mouse._startIndex.x]->_index.x << endl;
	}
...
void TileMap::render()
{
	...
	{
		if (_vTile[i]->_terrain)
		_terrainImg->FrameRender(Vector2(TILESIZE/2 + TILESIZE * (i % (int)_count.x), TILESIZE / 2 + TILESIZE * (i / (int)_count.x)), 0, (int)_vTile[i]->_terrain->_terrainType * 2, CAMERAMANAGER->GetVCamera()[0]);
	...
	{
		if (_vTile[i]->_wall)
			_vTile[i]->_wall->_img->FrameRender(Vector2(TILESIZE / 2 + TILESIZE * (i % (int)_count.x), TILESIZE / 2 + TILESIZE * (i / (int)_count.x)), _vTile[i]->_wallIndex%7, _vTile[i]->_wallIndex/7, CAMERAMANAGER->GetVCamera()[0]);
	}
...
void TileMap::SetTileMap(Vector2 count)
{
	...
	for (int i = 0; i < _tileCount; i++)
	{
		...
		_vTileObj.push_back(tile->_terrain);
		_vTile.push_back(tile);

		BuildTileObject(Vector2(tile->_index.x, tile->_index.y), new TileObject((TILETERRAIN)RND->getInt(5)));
	}
}
...
void TileMap::BuildTileObject(Vector2 index, TileObject * tileObj)
{
	switch (tileObj->_type)
	{
		...
		{
			//예외에 해당하면 만들지 않음.
			for (int i = 0; i < tileObj->_count.x; i++)
			{
				for (int j = 0; j < tileObj->_count.y; j++)
				{
					//이미 오브젝트가 있으면 만들지 않음.
					if (_vTile[(index.y + j) * _count.x + index.x + i]->_tileObj) return;
				}
			}
			...
			{
				//타입이 같다면 벽 타입이 같다면 만들지 않음.
				if (_vTile[index.y * _count.x + index.x]->_wall->_wallType == tileObj->_wallType) return;
		...
		{
			//예외에 해당하면 만들지 않음.
			if (tileObj->_direction == DIRECTION::FRONT || tileObj->_direction == DIRECTION::BACK)
			{
				for (int i = 0; i < tileObj->_count.x; i++)
				{
					for (int j = 0; j < tileObj->_count.y; j++)
					{
						...
						if (_vTile[(index.y + j) * _count.x + index.x + i]->_wall || _vTile[(index.y + j) * _count.x + index.x + i]->_tileObj) return;
			...
			{
				for (int i = 0; i < tileObj->_count.y; i++)
				{
					for (int j = 0; j < tileObj->_count.x; j++)
					{
						//이미 벽이나 오브젝트가 있으면 만들지 않음.
						if (_vTile[(index.y + j) * _count.x + index.x + i]->_wall || _vTile[(index.y + j) * _count.x + index.x + i]->_tileObj) return;
						//범위가 타일맵을 넘어가면 만들지 않음.
			...
			//해당 타일에 인덱스를 만들고 오브젝트의 주인으로 설정한다. (0)
			//카운트 인덱스를 전부 해당 오브젝트로 링크 시켜 준다.

			tileObj->_startIndex = index;

			//방향이 오른쪽 또는 왼쪽이라면 차지하는 x, y 칸의 개수를 뒤집어 준다.
			if (tileObj->_direction == DIRECTION::FRONT || tileObj->_direction == DIRECTION::BACK)
			{
				for (int i = 0; i < tileObj->_count.x; i++)
				{
			...
			else if (tileObj->_direction == DIRECTION::RIGHT || tileObj->_direction == DIRECTION::LEFT)
			{
				for (int i = 0; i < tileObj->_count.y; i++)
				{
					for (int j = 0; j < tileObj->_count.x; j++)
